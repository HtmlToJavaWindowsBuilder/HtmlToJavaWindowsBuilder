...
public class FontProperty extends JPanel implements CSSPainter {
    ...
    public void paintComponent(Graphics g) {
        ...
        int fontwidth = 100;
        int textIndent = 0;//= setTextIndent();
        ...
        //this.fontd.drawString(fontAttr.getIterator(), 35, 25);
    ...
    private int _setFontSize() {
        int size = 12;
        int variantSize = _setVariant(); 
        size = size + variantSize;
        if(this.property.get("font-size") == null) {
            return size;
        }
        else {
            return Integer.parseInt(this.property.get("font-size"));         
        }
    }
    ...
    /*
    public void testPaint(int test1) {
        switch(test1){
            case 1:{
                this.text = "abcc"
                break; 
